...
if "projection_assumptions" not in st.session_state:
    st.session_state.projection_assumptions = {
        "savings_interest": 0.035,
        "mutual_funds_cagr": 0.12,
        "fixed_deposits_cagr": 0.065,
        "stocks_cagr": 0.10,
        "gold_cagr": 0.07,
        "sip_cagr": 0.12,
    }

projection_assumptions = st.session_state.projection_assumptions
...
if mode == "🏠 Demo Customer":
    ...
    report_twin = build_financial_twin(report_customer, projection_assumptions)
    ...
    projection = report_twin["projections"]
    ...
    st.caption(
        "Projection includes savings interest, mutual fund CAGR, FD growth, stock growth, gold appreciation, and SIP compounding."
    )
    ...
    coach_twin = build_financial_twin(coach_customer, projection_assumptions)
...
twin = build_financial_twin(customer, projection_assumptions)
...
projection_df = pd.DataFrame({
    ...
    "Projected Value": [
        projected_values["savings"],
        projected_values["mutual_funds"],
        projected_values["fixed_deposits"],
        projected_values["stocks"],
        projected_values["gold"],
        projected_values["sip_corpus"]
    ]
})
